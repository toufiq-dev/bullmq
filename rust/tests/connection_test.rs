...
async fn test_connect_via_typed_options() {
    ...
    let (host, port, tls) = match conn_info.addr() {
        ...
        _ => panic!(
            "test_connect_via_typed_options only supports TCP/TLS REDIS_URL connection addresses"
        ),
    };

    let redis_settings = conn_info.redis_settings();
    let db = u8::try_from(redis_settings.db()).ok();
    let username = redis_settings.username().map(str::to_owned);
    let password = redis_settings.password().map(str::to_owned);
