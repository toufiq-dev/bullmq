...
In order to use the full potential of Bull queues, it is important to understand the lifecycle of a job. From the moment a producer calls the [`add`](https://docs.bullmq.io/api/classes/v6.Queue.html#add) method on a `Queue` instance, a job enters a lifecycle where it will be in different states, until its completion or failure (although technically a failed job could be retried and get a new lifecycle).
...
Another way to add a job is by the [`add`](https://docs.bullmq.io/api/classes/v6.FlowProducer.html#add) method on a flow producer instance.
