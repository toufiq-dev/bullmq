/**
 * Redis-only tests for `Queue.obliterate`.
 *
 * These verify obliteration by asserting on the raw Redis keyspace
 * (`client.keys('<prefix>:<queue>:*')`), which has no backend-agnostic
 * equivalent, and they exercise the flow-aware parent handling (a child queue
 * being obliterated moving/keeping its parent in another queue) that the
 * PostgreSQL flows slice does not implement yet. The backend-agnostic parts of
 * obliterate (the active-jobs guard, job-log removal) live in
 * `obliterate.test.ts`.
 */
...
describe('Obliterate (redis-only)', () => {
