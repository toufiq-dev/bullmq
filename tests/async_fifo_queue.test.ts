...
describe('AsyncFIFOQueue', () => {
  it('empty queue', async () => {
    ...
    expect(asyncFifoQueue.numPending()).toEqual(0);
    expect(asyncFifoQueue.numQueued()).toEqual(0);
    expect(asyncFifoQueue.numTotal()).toEqual(0);
  ...
  it('add several promises and wait for them to complete', async () => {
    ...
    expect(asyncFifoQueue.numPending()).toEqual(promises.length);
    expect(asyncFifoQueue.numQueued()).toEqual(0);
    expect(asyncFifoQueue.numTotal()).toEqual(promises.length);

    await asyncFifoQueue.waitAll();
    expect(asyncFifoQueue.numPending()).toEqual(0);
    expect(asyncFifoQueue.numQueued()).toEqual(promises.length);
    expect(asyncFifoQueue.numTotal()).toEqual(promises.length);
  });
  ...
  it('add several promises with random delays and wait for them to complete in order', async () => {
    ...
    expect(results).toEqual(randomDelays.sort((a, b) => a - b));
  });
  ...
  it("should handle promises that get rejected and don't block the queue", async () => {
    ...
    expect(results).toEqual(randomDelays.map(() => void 0));
  });
});
