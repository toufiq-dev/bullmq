...
In Hex packages and Mix releases, BullMQ reads these SQL files from
`elixir/priv/postgres`; when running from this repository during development, it
falls back to `src/postgres`. Migrations are applied automatically by
`BullMQ.Backends.Postgres.Connection.start_link/1`.
...
- PostgreSQL 13+ (PostgreSQL backend)
